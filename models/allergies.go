...
func init() {
	orm.RegisterModel(new(Allergies))
}

// AddAllergies insert a new Allergies into database and returns
// last inserted ID on success.
func AddAllergies(m *Allergies) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}
...
// GetAllAllergies retrieves all Allergies matches certain condition. Returns empty list if
// no records exist
func GetAllAllergies(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Allergies))
	// query k=v
	for k, v := range query {
...
// UpdateAllergies updates Allergies by ID and returns error if
// the record to be updated doesn't exist
func UpdateAllergiesById(m *Allergies) (err error) {
	o := orm.NewOrm()
	v := Allergies{ID: m.ID}
...
func DeleteAllergies(id string) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Allergies{ID: id}); err == nil {
