	...
	function getClosestIndex(x, columnWidths, __affixedColumnIndices, __scrollLeft) {
		...
		if (__affixedColumnIndices.length > 0) {
			...
			const lastAffixedRight =
				lastAffixedLeft + columnWidths[__affixedColumnIndices[__affixedColumnIndices.length - 1]];
...
<svelte:window on:keydown={onWindowKeyDown} on:mousemove={onMouseMove} on:mouseup={onMouseUp} />
<div
	bind:this={wrapper}
...
>
	{#if __resizing || __columnDragging || __affixingColumn}
		<div class="column-action-line" style="left: {__columnActionLineLeft - 2}px;" />
	{/if}
	{#if __affixingRow}
		<div class="row-action-line" style="top: {__rowActionLineTop - 2}px;" />
	{/if}

	<div class="grid-headers" rolw="rowgroup" style="height: {rowHeight}px;">
					...
					>
						...
						{:else}
							<div
								style={column.style}
								class={`cell-default ${
									row.i % 2 === 0 && Striped && !(CurrentSelectedRow === row.i && EnableCursor)
										? ' stripedRow '
										: ''
								} ${CurrentSelectedRow === row.i && EnableCursor ? 'selectedrow' : ''}`}
							>
