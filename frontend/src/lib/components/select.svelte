...
<div class="border rounded p-2 border-dark-green ">
    <div class='relative text-medium-black px-3 text-lg cursor-pointer' on:click={() => showSelect = true}>
    ...
    {#if showSelect}
        <input bind:value={val} type="text" id="searchInput"
               class="drop-shadow-lg focus:shadow-dark-green border  rounded text-medium-black px-2 w-full text-md p-1 border-light-green focus:border-dark-green focus:ring-dark-green"
               autofocus>
        {#each $filtered as item}
            <div on:click={() => {val = item;showSelect=false}}
                 class=" w-full text-medium-black px-3 text-lg p-2 border-medium-black focus:border-dark-green focus:ring-dark-green cursor-pointer hover:bg-dark-gray rounded ">
                {item}
            </div>
        {/each}
    {/if}
